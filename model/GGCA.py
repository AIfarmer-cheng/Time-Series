...
class GGCA(nn.Module):
    def __init__(self, channel, h, w, reduction=16, num_groups=4): # reduction: 通道降维比例
        ...
        # 定义H方向的全局平均池化和最大池化
        # 由輸出的尺寸自動計算窗口的尺寸，然後在窗口内進行平均和取最大值
        self.avg_pool_h = nn.AdaptiveAvgPool2d((h, 1))  # 输出为[batch_size, group_channels, h, 1]，只改变空间维度
